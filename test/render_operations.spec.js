describe('renderOperations', function () {
	...
	describe('delete', function () {
		beforeEach(function () {
			var before = tokenize(['this', ' ', 'is', ' ', 'a', ' ', 'test', ' ', 'of', ' ', 'stuff']);
	...
	describe('replace', function () {
		...
		it('should wrap in both <ins> and <del>', function () {
			expect(res).to.equal(
				'this is a <del data-operation-index="1">break</del>' + '<ins data-operation-index="1">test</ins>',
			);
		});
	});
	...
	describe('Dealing with tags', function () {
		...
		it('should identify contained inserted tags', function () {
			expect(res).to.equal(
				'<p>a<ins data-operation-index="1"> b</ins></p>' +
		...
		it('should identify contained deleted tags', function () {
			...
			expect(res).to.equal(
				'<p>a<del data-operation-index="1"> b</del></p>' +
		...
		it('should not identify partial tags', function () {
			...
			expect(res).to.equal(
				'<del data-operation-index="0">test</del>' +
		...
		describe('When there is a change at the beginning, in a <p>', function () {
			...
			it('should keep the change inside the <p>', function () {
				expect(res).to.equal(
					'<p><del data-operation-index="1">this</del>' + '<ins data-operation-index="1">I</ins> is awesome</p>',
				);
			});
		});
	});
	...
	describe('tags with attributes', function () {
		...
		it('should show changes within tags with different attributes', function () {
			...
			expect(res).to.equal(
				'<p style="margin: 2px;" class="after">' +
	...
	describe('wrappable tags', function () {
		it('should wrap void tags', function () {
			...
			expect(res).to.equal(
				'<del data-operation-index="0">old</del>' + '<ins data-operation-index="0">new<br/></ins> text',
			);
		});
